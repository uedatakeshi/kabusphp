...
function calcFourth($output, $loop_array) {
    ...
    if (isset($output[$c3]['openingprice'])) {
        $drate = 100 * ($output[$c3]['price'] - $output[$c3]['openingprice']) / $output[$c3]['openingprice'];
    } else {
        return false;
    }
    /*
    if (isset($output[$c3]['time']) && preg_match("/^(\d+)-(\d+)-(\d+) (\d+):(\d+):(\d+)/", $output[$c3]['time'], $regs)) {
        $h = $regs[4];
        $m = $regs[5];
        $b = $regs[6];
        $c_time = mktime($h, $m, $b);
    ...
    */
    ...
    $lrate = 0;
    if (isset($output[$c3]['lowprice'])) {
        $lrate = 100 * ($output[$c3]['price'] - $output[$c3]['lowprice']) / $output[$c3]['lowprice'];
    } else {
        return false;
    }

    if ((($diff1 - $diff2) > 1) && ($diff1 > $diff2) && ($diff2 > 0)) {
        if (($vdiff1 > $vdiff2) && ($vdiff1 > 10000)) {
//            if (($wrate < 101.6) && ($prate > 0.4) && ($lrate > 1)) {
            if (($wrate < 101.6) && ($prate > 0.4)) {
                ...
                return "{$output[$c4]['time']}," . 
                    "{$diff1}, {$diff2}, " . 
                    "$prate, {$output[$c3]['price']}, $drate, " . 
                    "{$output[$c3]['openingprice']}, {$output[$c3]['lowprice']}, " .
                    "{$output[$c3]['vwap']}, {$wrate}, " . 
                    "{$output[$c3]['changepreviouscloseper']}, " .
                    "$vdiff1, $vdiff2, {$output[$c3]['tradingvolume']}, $vrate, $lrate";
